...
// Runtime integrity protection modules
#[cfg(target_os = "linux")]
    ...
    pub fn get_process_power_consumption_microwatts(&self, pid: Pid) -> Option<Record> {
        if let Some(record) = self.get_proc_tracker().get_process_last_record(pid) {
            ...
            if let Some(conso) = &topo_conso {
                ...
                // Use VM SGX enclave for trusted calculation if available
                #[cfg(feature = "use_sgx_vm")]
                {
                    use crate::sensors::powercap_rapl::is_sgx_initialized;
                    
                    if is_sgx_initialized() {
                        // Call SGX enclave for protected computation
                        ...
                        if status == 0 {
                            trace!("[VM-SGX] Process {} energy computed in SGX enclave: {} uW", pid, sgx_result);
                            return Some(Record::new(
                                record.timestamp,
                ...
                // Fallback: standard calculation (or if SGX not available)
                let result =
...
pub struct Record {
    ...
    pub value: String,
    pub unit: units::Unit,
    /// HMAC-SHA256 signature of the value (for attestation)
    #[cfg(any(feature = "tpm_attestation", feature = "tpm_attestation_vm"))]
...
impl Record {
    ...
    /// Create Record with HMAC signature
    #[cfg(any(feature = "tpm_attestation", feature = "tpm_attestation_vm"))]
